...
/// <summary>
/// Service class providing access to API endpoints for TokenService.
...
{
    private RequestConfig? _generateTokenAsyncConfig;

    internal TokenService(Client httpClient)
        : base(httpClient) { }

    /// <summary>
    /// Sets method-level configuration for <c>GenerateTokenAsync</c>.
    /// Method-level config overrides service-level config but is overridden by per-request config.
    /// </summary>
    public TokenService SetGenerateTokenAsyncConfig(RequestConfig config)
    {
        _generateTokenAsyncConfig = config;
        return this;
    }
    ...
    )
    {
        ...
        var resolvedConfig = GetResolvedConfig(_generateTokenAsyncConfig, requestConfig);
